...
TRAIN_DATASET = os.path.join(os.path.dirname(os.path.abspath(__file__)), "label_encoded.csv")
TEST_DATASET = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_dataset.csv")
# Train dataset
...
#X_train = X_train.join([bagofwordsDataframe]) #
#X_train = X_train.drop(['ipSrc'],axis=1)
#X_train = X_train.drop(['ipDst'],axis=1)
...
testDataset.head()
"""
testDetail = testDataset['detail']
listOfStrings = wordList()
testDetail = removeChar(testDetail)
vectorizer = CountVectorizer(vocabulary=listOfStrings)
bagofwordsDetail = vectorizer.fit_transform(testDetail).toarray()
bagofwordsDataframeDetail = pd.DataFrame(data = bagofwordsDetail, columns = listOfStrings)
"""
X_test = testDataset.drop(['deviceName'],axis=1)
X_test = X_test.drop(['detail'],axis=1)
#X_test = X_test.join([bagofwordsDataframeDetail]) #
#X_test = X_test.drop(['ipSrc'],axis=1)
#X_test = X_test.drop(['ipDst'],axis=1)
