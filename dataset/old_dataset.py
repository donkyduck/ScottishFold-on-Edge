...
TRAIN_PAPER = os.path.join(os.path.dirname(os.path.abspath(__file__)), "train_paper_grouped.db")
            ...
            for order in range(0,len(cap)):
                ...
                try:
                    if str(currentCap.highest_layer) == str(tempProtocol):
                        ...
                        ipSrc = getIP(currentCap,protocolChk,"src")
                        ipDst = getIP(currentCap,protocolChk,"dst")
...
def pushToDB(ipSrc,ipDst,macSrc,macDst,portSrc,portDst,pktLength,deviceName,protocolID,detail):
    try:
        ...
        print(ipSrc + ", " + macDst + ", " + portSrc + ", " + portDst + ", " + pktLength + ", " + deviceName + ", "  + protocolID)
        cursor.close()
    except:
...
def getIP(currentCap,protocolChk,srcOrDst):
    subsIP = ":ip:"
    subsIPv6 = ":ipv6:"
    if subsIP in protocolChk and srcOrDst == "src":
        srcIP = str(currentCap.ip.src)
        return srcIP
    elif subsIP in protocolChk and srcOrDst == "dst":
        dstIP = str(currentCap.ip.dst)
        return dstIP
    elif subsIPv6 in protocolChk and srcOrDst == "src":
        srcIP = str(currentCap.ipv6.src)
        return srcIP
    elif subsIPv6 in protocolChk and srcOrDst == "dst":
        dstIP = str(currentCap.ipv6.dst)
        return dstIP
    else:
        pass
...
def getDeviceName(macAddr):
    ...
    elif "4c:ef:c0:a9:b1:c1" in macAddr:
...
def fileList():
    fileInDirectory = glob.glob("/home/meowmeow/Desktop/Database/ECHO/*.pcapng")
